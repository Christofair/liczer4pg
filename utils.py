...
def collect_posts_from_topic(topic_html_doc: str) -> list[html.Element]:
    ...
    for post in posts:
        ...
        if any([re.search('.*wyniki.*png', img.get('alt')) for img in imgs]):
            posts.remove(post)
            break
        elif any([re.search('image.png.35a820dc36f8f7f5203f0c731661b385.png', img.get('src')) for
                  img in imgs]):
            posts.remove(post)
            break
        elif any([re.search('typowanieinnykolor2.png.d3509b73d73f36d87c4163016b44e81a.png',
